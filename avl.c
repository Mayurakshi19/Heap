// AVL TREE Implementation-Conversion of Binary Search Tree into AVL Tree using LL,LR,RL and RR Rotations.
   #include<stdio.h>
   #include<stdlib.h>
...
int height(struct student *node)
{
    if (node == NULL)
        return -1;
    else {
...
return height(N->left) - height(N->right);
   ...
   struct student *add_loc(int nrank)
   {
   if(nrank<node->rank)
   {
...
if (balance > 1 && nrank < node->left->rank)
    rightRotate(node);

if (balance < -1 && nrank > node->right->rank)
    leftRotate(node);
